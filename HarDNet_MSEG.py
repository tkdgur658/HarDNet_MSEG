...
class HarDNet(nn.Module):
    def __init__(self, in_channels, num_classes, depth_wise=False, arch=85, pretrained=True, weight_path=''):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        max_pool = True
...
def hardnet(in_channels=1, num_classes=1, arch=68,pretrained=False, **kwargs):
    if arch ==68:
        model = HarDNet(in_channels=in_channels, num_classes=num_classes, arch=68)
        if pretrained:
            weights = torch.load('models/HardNet_lib/hardnet68.pth')
    ...
    #    if pretrained:
    #        print("HAAAHAAA")
    #        weights = torch.load('lib/hardnet85.pth')
    #        model.load_state_dict(weights)
    return model
...
class HarDNet_MSEG(nn.Module):
    # res2net based encoder decoder
    def __init__(self, in_channels=1, num_classes=1, channel=32):
        super(HarDNet_MSEG, self).__init__()
        # ---- ResNet Backbone ----
        ...
        self.hardnet = hardnet(arch=68,in_channels=in_channels, num_classes=num_classes)
    ...
    def forward(self, x):
        ...
        lateral_map_5 = F.interpolate(ra5_feat, scale_factor=8, mode='bilinear',align_corners=True)    # NOTES: Sup-1 (bs, 1, 44, 44) -> (bs, 1, 352, 352)

        return lateral_map_5 #, lateral_map_4, lateral_map_3, lateral_map_2

# model = HarDNet_MSEG(1).cuda()
# model(torch.rand((2, 1, 256, 256)).cuda()).shape
